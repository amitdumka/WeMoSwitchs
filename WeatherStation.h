...
//This supported now ESP8266 and Wityy Cloud .
//Will add supoort for ESP32
...
String OPEN_WEATHER_MAP_APP_ID = "59043edc370e0751a311f35e1d2266ec"; //AMI
...
String OPEN_WEATHER_MAP_LOCATION_ID = "1272237"; //Dumka
...
*/
    ...
    // TODO: Need to update code for Correct time zone and daylighy saving for India/ your location
...
void WeatherStationLoop()
{
    ...
    if ((millis() - timeSinceLastWUpdate) > 5000)
    {
        ReadDHTSenor();
        timeSinceLastWUpdate = millis();
    }
...
void drawCurrentRoomTemp(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    display->setFont(ArialMT_Plain_10);
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    String humd = "Humidity : " + String(Humidity, 1) + "% HI :" + String(HeatIndex, 1) + "°C";
    display->drawString(64 + x, 38 + y, humd);

    display->setFont(ArialMT_Plain_24);
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    String temp = String(Temp_C, 1) + (IS_METRIC ? "°C" : "°F");
    display->drawString(60 + x, 5 + y, temp);
